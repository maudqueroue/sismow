...
* different types of survey `design` such as parallel `systematic`, zigzag `eszigzag`, crossed zigzag `eszigzagcom` or `random` transects       
* total transect length (approximately) `line.transect`      
* angle of the transects `design angle`      
* segmentation of the transects `segmentize`
* length of segments (approximately) `length_segs`
* and others : see `?make.design` for `dssd` package    

The output is a sf dataframe containing the simulated transects corresponding. The dataframe contains four columns: `transect` number, `seg_ID` (a unique identifiant for each segments/transect), `effort` (length of each segment/transect) and `geometry`.
...
For this example, `shape_courseulles`, a dataframe included in this package is used. Then, with `simulate_transect` some transect design are simulated: 
- (1) systematic parallel transects with a approximative total length of 400000m
...
- (3) the `shape_obj` is a density map simulated with `simulate_density` and the design is systematic parallel transects with a approximative total length of 400000m that are segmentized with a length of approximatively 2000m per segments.

```{r examples-simulate_transects}
